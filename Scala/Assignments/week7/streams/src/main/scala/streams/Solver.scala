...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    lazy val legalNeighborMoves: List[(Block, List[Move])] =
      for {
        neighbor <- b.legalNeighbors
      } yield (neighbor._1, neighbor._2 :: history)

    legalNeighborMoves.toStream
  }
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    def newNeighbour(neighbor: (Block, List[Move])): Boolean = !(explored contains neighbor._1)

    neighbors filter newNeighbour
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, moves) #:: rest => {
      val exploredWithBlock = explored + block   // only evaluate once 
      val more = newNeighborsOnly(neighborsWithHistory(block, moves), exploredWithBlock)
      
      more ++ from(rest ++ more, exploredWithBlock)
    }
  }
